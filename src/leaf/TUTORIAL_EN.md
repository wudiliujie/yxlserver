...
* Multi-core support. Leaf utilize its modules and [leaf/go](https://leaf/tree/master/go) to make use of CPU resouces at maximum while avoiding varieties of side effects may be caused.
...
* Modules communicate with one another via a light weight RPC channel([leaf/chanrpc](https://leaf/tree/master/chanrpc))
...
More references are at [leaf/go](https://leaf/blob/master/go)。
...
More references are at [leaf/timer](https://leaf/blob/master/timer)。
...
More references are at [leaf/log](https://leaf/blob/master/log).

### Leaf recordfile

Leaf recordfile is formatted in CSV([Example](https://leaf/blob/master/recordfile/test.txt)). recordfile is to manage the configuration for game. The usage of recordfile in LeafServer is quite simple:
...
Refer to [leaf/recordfile](https://leaf/blob/master/recordfile) for more details.

Learn more
----------

More references are at Wiki [https://leaf/wiki](https://leaf/wiki)
